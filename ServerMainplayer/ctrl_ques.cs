...
namespace SQL
{
    class ctrl_ques
    {
        ...
        {
            string sql = "SELECT ID_UV, NOIDUNG, DUNG, DA1, DA2, DA3, DA4, DA5, DA6 FROM QUESTION_UNGVIEN ORDER BY RAND() LIMIT 1";
            sqldata.getconnect().Open();
            using (SqlDataAdapter adapter = sqldata.getadapter(sql))
            {
                adapter.Fill(ds, "dsQUESTION_UV");
                posques_ungvien.Id = ds.Tables["dsQUESTION_UV"].Rows[0]["ID_MP"].ToString();
